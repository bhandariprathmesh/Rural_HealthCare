...
-- AlterTable
ALTER TABLE "EmergencyAccessLog"
ADD COLUMN IF NOT EXISTS "sosAlertId" TEXT;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'SosAlert_raisedByWorkerId_fkey'
    ) THEN
        ALTER TABLE "SosAlert"
        ADD CONSTRAINT "SosAlert_raisedByWorkerId_fkey"
        FOREIGN KEY ("raisedByWorkerId")
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'SosAlert_facilityId_fkey'
    ) THEN
        ALTER TABLE "SosAlert"
        ADD CONSTRAINT "SosAlert_facilityId_fkey"
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'SosEscalationEvent_sosAlertId_fkey'
    ) THEN
        ALTER TABLE "SosEscalationEvent"
        ADD CONSTRAINT "SosEscalationEvent_sosAlertId_fkey"
        FOREIGN KEY ("sosAlertId")
        REFERENCES "SosAlert"("id")
        ON DELETE CASCADE
        ON UPDATE CASCADE;
    END IF;
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'EmergencyAccessLog_sosAlertId_fkey'
    ) THEN
        ALTER TABLE "EmergencyAccessLog"
        ADD CONSTRAINT "EmergencyAccessLog_sosAlertId_fkey"
        FOREIGN KEY ("sosAlertId")
        REFERENCES "SosAlert"("id")
        ON DELETE SET NULL
        ON UPDATE CASCADE;
