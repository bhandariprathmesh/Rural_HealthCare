...
export async function saveOfflineConsultation(
  data: any,
): Promise<OfflineConsultation> {
...
export async function saveOfflineAppointment(
  data: any,
): Promise<OfflineAppointment> {
  const id = data.id || `TEMP-APT-${Date.now()}`

  const appointmentRecord: OfflineAppointment = {
    id,
    ...
    tokenNumber: data.tokenNumber || Math.floor(1 + Math.random() * 50),
  ...
  await offlineDb.appointments.put(appointmentRecord)
  ...
  try {
    ...
    for (const item of queue) {
      ...
      try {
        if (item.action === "CREATE_PATIENT") {
          console.log(
            `[SyncEngine] Syncing item #${item.id} (${item.action}) -> POST /api/v1/patients/register`,
          )
          ...
          const patientName =
            rawPayload.name?.trim() && rawPayload.name.trim().length >= 2
          ...
          const dob =
            rawPayload.dob && String(rawPayload.dob).trim().length >= 4
          ...
          if (
            !gender ||
            !["Male", "Female", "Other", "M", "F", "O"].includes(gender)
          ) {
            gender = "Other"
          }
          ...
          const village =
            rawPayload.village?.trim() && rawPayload.village.trim().length >= 2
              ? rawPayload.village.trim()
              : "Bikaner Rural"

          const district =
            rawPayload.district?.trim() &&
            rawPayload.district.trim().length >= 2
              ? rawPayload.district.trim()
              : "Bikaner"

          const state =
            rawPayload.state?.trim() && rawPayload.state.trim().length >= 2
          ...
          const bloodGroup =
            rawPayload.bloodGroup || rawPayload.blood || "Not known"
          ...
          const consent = {
            ...
            ...(rawPayload.consent?.purpose
              ? { purpose: rawPayload.consent.purpose }
              : {}),

            ...(Array.isArray(rawPayload.consent?.dataScope)
              ? { dataScope: rawPayload.consent.dataScope }
              : {}),
          }
          ...
          const allergies = Array.isArray(rawPayload.allergies)
            ...
            : typeof rawPayload.allergies === "string" &&
                rawPayload.allergies.trim()
              ? rawPayload.allergies
                  .split(",")
                  .map((s: string) => s.trim())
                  .filter(Boolean)
          ...
          const chronicConditions = Array.isArray(rawPayload.chronicConditions)
            ...
            : typeof rawPayload.chronicConditions === "string" &&
                rawPayload.chronicConditions.trim()
              ? rawPayload.chronicConditions
                  .split(",")
                  .map((s: string) => s.trim())
                  .filter(Boolean)
          ...
          const currentMedications = Array.isArray(
            rawPayload.currentMedications,
          )
            ? rawPayload.currentMedications
            : typeof rawPayload.currentMedications === "string" &&
                rawPayload.currentMedications.trim()
              ? rawPayload.currentMedications
                  .split(",")
                  .map((s: string) => s.trim())
                  .filter(Boolean)
          ...
          if (
            ...
            const ecPhone = String(rawPayload.emergencyContact.phone || "")
              .replace(/\D/g, "")
              .slice(-10)
          ...
          const age =
            typeof rawPayload.age === "number" && rawPayload.age > 0
              ? rawPayload.age
              : undefined
          ...
          const apiPayload: any = {
            ...
            ...(rawPayload.nameHi?.trim()
              ? { nameHi: rawPayload.nameHi.trim() }
              : {}),
            ...
            ...(rawPayload.abhaAddress?.trim()
              ? { abhaAddress: rawPayload.abhaAddress.trim() }
              : {}),

            ...(rawPayload.healthWorkerId
              ? { healthWorkerId: rawPayload.healthWorkerId }
              : {}),

            ...(rawPayload.healthWorkerName
              ? { healthWorkerName: rawPayload.healthWorkerName }
              : {}),
          ...
          try {
            ...
            if (
              ...
              res = {
                patient: { healthId: matchedHealthId, id: matchedHealthId },
              }

              console.log(
                `[SyncEngine] Patient already registered on backend (${matchedHealthId || cleanPhone}). Recovered real ID.`,
              )
            } else {
              throw apiErr
            }
          ...
          if (localId && realPatientId && localId !== realPatientId) {
            const tempIds = new Set(
              [localId, item.payload.id, item.payload.healthId].filter(Boolean),
            )
            ...
            // 4. Update queued CREATE_APPOINTMENT items in memory and IndexedDB

            for (const remainingItem of queue) {
              if (
                remainingItem.action === "CREATE_APPOINTMENT" &&
                remainingItem.payload &&
                tempIds.has(remainingItem.payload.patientId)
              ) {
                remainingItem.payload.patientId = realPatientId
            ...
            const queuedAppointments = await offlineDb.outboxQueue

              .filter(
                (qItem) =>
                  qItem.action === "CREATE_APPOINTMENT" &&
                  qItem.payload &&
                  tempIds.has(qItem.payload.patientId),
              )
            ...
            for (const qa of queuedAppointments) {
              if (qa.id !== undefined) {
                await offlineDb.outboxQueue.update(qa.id, {
                  payload: {
                    ...qa.payload,
          ...
          console.log(
            `[SyncEngine] Successfully synced item #${item.id} (${item.action}). Real ID: ${realPatientId}`,
          )

          processed++
        } else if (item.action === "CREATE_CONSULTATION") {
          console.log(
            `[SyncEngine] Syncing item #${item.id} (${item.action}) -> POST /api/v1/consultations`,
          )
          ...
          const apiPayload: any = {
            ...
            symptoms: Array.isArray(rawPayload.symptoms)
              ? rawPayload.symptoms
              : [],

            vitals:
              typeof rawPayload.vitals === "object" && rawPayload.vitals
                ? rawPayload.vitals
                : {},
            ...
            prescription: Array.isArray(rawPayload.prescription)
              ? rawPayload.prescription
              : [],
          ...
          if (localIdToUpdate) {
            await offlineDb.consultations.update(localIdToUpdate, {
              synced: true,
            })
          }

          console.log(
            `[SyncEngine] Successfully synced item #${item.id} (${item.action}).`,
          )

          processed++
        } else if (item.action === "CREATE_APPOINTMENT") {
          console.log(
            `[SyncEngine] Syncing item #${item.id} (${item.action}) -> POST /api/v1/appointments`,
          )
          ...
          const apiPayload = {
            ...
            facilityId: rawPayload.facilityId,

            scheduledDate: rawPayload.scheduledDate,

            timeSlot: rawPayload.timeSlot || "10:00 AM - 11:00 AM",

            reason: rawPayload.reason,

            priority: rawPayload.priority,

            source: rawPayload.source || "PATIENT",

            bookedByWorkerId: rawPayload.bookedByWorkerId,
          }

          const booked = await bookAppointment(apiPayload)
          ...
          console.log(
            `[SyncEngine] Successfully synced appointment #${item.id}`,
          )
        ...
        console.error(
          `[SyncEngine] Failed to sync outbox item #${item.id} (${item.action}):`,
          lastSyncError,
        )
    ...
    console.log(
      `[SyncEngine] flushOutbox finished. Processed: ${processed}, Errors: ${errors}`,
    )
  } catch (err: any) {
